...
# clf = NearestCentroid()
clf = ClassifLibrary.ClfType.LINEAR
...
clfs = ClassifLibrary.initialize_classifiers(number_of_classifiers, clf)
...
if are_samples_generated:
    ...
    X0, X1 = ClassifLibrary.divide_generated_samples(X, y)
...
X_unsplitted, y_unsplitted, X_final_test, y_final_test = ClassifLibrary.split_sorted_samples_between_classifiers(X, y, number_of_classifiers)

# Splitting between training and testing
print('Split between training and testing')
scores, coefficients = [], []
Xs_train, Xs_test, ys_train, ys_test = ClassifLibrary.split_sorted_samples_between_training_and_testing(X_unsplitted, y_unsplitted, training2testing_quotient)

# Getting data for plot
print('Get data for plot')
x_min, x_max, y_min, y_max = ClassifLibrary.get_samples_limits(X)
...
for clf, X_train, X_test, y_train, y_test in zip(clfs, Xs_train, Xs_test, ys_train, ys_test):
    ...
    type_of_classifier = ClassifLibrary.determine_clf_type(clf)
    # Testing classifier for every subspace
    for j in range(number_of_space_parts):
        X_part, y_part = ClassifLibrary.prepare_samples_for_subspace(X_test, y_test, X, j, number_of_space_parts)
        if len(X_part) > 0:
            score.append(clf.score(X_part, y_part))
        else:
            score.append(0)
    scores.append(score)
    if type_of_classifier == ClassifLibrary.ClfType.LINEAR:
        a, b = ClassifLibrary.extract_coefficients_for_linear(clf)
    elif type_of_classifier == ClassifLibrary.ClfType.MEAN:
        a, b = ClassifLibrary.extract_coefficients_for_mean(clf)
...
score, flip_index = [], 0
for j in range(number_of_space_parts):
    x_subspace_min, x_subspace_max = ClassifLibrary.get_subspace_limits(X, j, number_of_space_parts)
    x = np.linspace(x_subspace_min, x_subspace_max)
    a, b = ClassifLibrary.evaluate_average_coefficients_from_n_best(coefficients, scores, j, number_of_best_classifiers, number_of_classifiers)
    y = a * x + b
    ax.plot(x, y)
    X_part, y_part = ClassifLibrary.prepare_samples_for_subspace(X_final_test, y_final_test, X, j, number_of_space_parts)
