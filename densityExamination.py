...
X, y = make_classification(n_features=2, n_redundant=0, n_informative=1, n_clusters_per_class=1,
                           n_samples=number_of_samples, class_sep=2.7, hypercube=False, flip_y=0, random_state=2)
...
for j in range(partitioning):
    ...
    a, b = my_library.evaluate_coefficients_from_n_best(coefficients, number_of_classifiers, scores, j, number_of_best_classifiers)
    y = a * x + b
    ax.plot(x, y)
    X_part, y_part = my_library.prepare_samples_for_subspace(X_final_test, y_final_test, X, j, partitioning)
    if (len(X_part) > 0):
